...
namespace SolastaAlchemistClass
{
    public class Main
    {
        ...
        {
            DirectoryInfo directoryInfo = new DirectoryInfo($@"{UnityModManager.modsPath}/SolastaAlchemistClass");
            FileInfo[] files = directoryInfo.GetFiles($"Translations-??.txt");

            foreach (var file in files)
            {
                var filename = $@"{UnityModManager.modsPath}/SolastaAlchemistClass/{file.Name}";
